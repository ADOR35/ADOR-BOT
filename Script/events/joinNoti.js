...
module.exports.run = async function({ api, event }) {
  ...
  const botName = global.config.BOTNAME || "TANBIR 𝗖𝗵𝗮𝘁 𝗕𝗼𝘁";

 
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    await api.changeNickname(`[ ${botPrefix} ] • ${botName}`, threadID, api.getCurrentUserID());

    api.sendMessage("চ্ঁলে্ঁ এ্ঁসে্ঁছি্ঁ TANBIR 𝐂𝐡𝐚𝐭 𝐁𝐨𝐭 এঁখঁনঁ তোঁমাঁদেঁরঁ সাঁথেঁ আঁড্ডাঁ দিঁবঁ..!", threadID, () => {
...
➤𝐌𝐞𝐬𝐬𝐞𝐧𝐠𝐞𝐫: https://m.me/61583233374337
➤𝐖𝐡𝐚𝐭𝐬𝐀𝐩𝐩: https://wa.me/+9660570434110

❖⋆═══════════════════════⋆❖
          𝐁𝐨𝐭 𝐎𝐰𝐧𝐞𝐫 ➢ TANBIR HOSSAIN`;
